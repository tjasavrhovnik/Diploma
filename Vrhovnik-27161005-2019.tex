...
\begin{dokaz}
$(\Rightarrow)$ 
Denimo, da sta ulomka $\frac{a}{b}$ in $\frac{c}{d}$ Fareyeva soseda v zaporedju $F_{n}$. Trdimo, da velja $bc - ad = 1$. Dokaz bo potekal z indukcijo na red Fareyevega zaporedja, torej na $n$.
...
$(\Leftarrow)$ 
Vzemimo okrajšane ulomke $\frac{a}{b}$, $\frac{p}{q}$ in $\frac{c}{d}$, ki naj bodo členi nekega Fareyevega zaporedja. Naj zanje velja $\frac{a}{b} <\frac{p}{q} < \frac{c}{d}$ in $bp - aq = qc - pd = 1$. S preureditvijo teh enakosti dobimo
\begin{align}
\label{Eq:LastFarSos}
bp + pd &= aq + qc, \nonumber \\
p(b + d) &= q(a + c), \nonumber \\
...
\end{align}
Če v obe enakosti iz predpostavke vstavimo $p=a+c$ in $q=b+d$, dobimo zvezo $b(a+c)-a(b+d)=(b+d)c-(a+c)d=bc-ad=1$. Trdimo, da sta ulomka $\frac{a}{b}$ in $\frac{c}{d}$ Fareyeva soseda v nekem Fareyevem zaporedju. Res, enakost~\ref{Eq:LastFarSos} nam pove, da je ulomek $\frac{p}{q}$ medianta ulomkov $\frac{a}{b}$ in $\frac{c}{d}$, iz definicije mediante pa sledi, da sta ta dva ulomka Fareyeva soseda.
\end{dokaz}
...
Funkcija $x \mapsto f(x)$ pripada razredu $O \left(g(x) \right)$, če absolutna vrednost funkcije $f$ ne raste hitreje od funkcije $g$, pomnožene s konstanto, ko gre $x$ proti $\infty$.
...
Sliki Fordovih krogov C($\frac{0}{1}$) in C($\frac{1}{0}$) bomo obravnavali posebej. Preslikava $z \mapsto -\frac{1}{z}$ je M\"obiusova transformacija, ki slika premice in krožnice v premice in krožnice, krožnica pa je natanko določena s tremi točkami. Zato v našem primeru zadostuje poznati slike treh točk na Fordovem krogu. Izberimo točke $0$, $i$ in $\frac{1}{2} + \frac{1}{2} i$, ki ležijo na Fordovem krogu C($\frac{0}{1}$). Preslikajo se v točke $-\infty$, $i$ in $-1+i$; te ležijo na premici $\{x+i; x \in \mathbb{R} \}$, to je Fordovemu krogu C($\frac{1}{0}$). Zato je slika kroga C($\frac{0}{1}$) Fordov krog C($\frac{1}{0}$). Ker je preslikava bijekcija, se Fordov krog C($\frac{1}{0}$) preslika v C($\frac{0}{1}$).
...
Funkcija $x \mapsto f(x)$ pripada razredu $o \left(g(x) \right)$, če absolutna vrednost funkcije $f$ raste počasneje od funkcije $g$, ko gre $x$ proti $\infty$.
...
Dokaz zgornje trditve izpustimo. (Sklic na vir, kjer bralec lahko najde dokaz, se nahaja v~\cite[References, str.~307, F1]{zetafunction}.) Pač pa bomo v nadaljevanju dokazali povezavo med navedenima ekvivalentnima formulacijama Riemannove hipoteze, ki jo lahko združimo v izrek.
...
\emph{M\"obiusova inverzija} se izraža s formulo
\begin{equation}
...
\bibitem{ford} L.~R.~Ford, \emph{Fractions}, v: The American Mathematical Monthly (ur.\ E.~J.~Moulton), \textbf{45}, Mathematical Association of America, 1938, str.\ 586--601; dostopno tudi na \url{https://www.maths.ed.ac.uk/~v1ranick/papers/ford.pdf}.
