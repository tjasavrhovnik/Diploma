...
\begin{definicija}
Naj bo $B_{n}$ $n$-ti Bernoullijev polinom. Pripadajoča funkcija $ \bar{B_{n}} $ je definirana kot $ \bar{B_{n}} = B_{n}(x - \lfloor x \rfloor). $
\end{definicija}
...
\begin{primer}
Funkcijo $ \bar{B_{1}} = x - \lfloor x \rfloor - \frac{1}{2} $ bomo potrebovali v dokazu.
\end{primer}

\begin{trditev}
Naj bo $k \in \mathbb{N}$. Tedaj je
\begin{equation}
B_{n}(kx) = k^{n-1} \left ( B_{n}(x) + B_{n} \left( x + \frac{1}{k} \right) + \dots + B_{n} \left( x + \frac{k-1}{k} \right) \right).
\end{equation}
\end{trditev}
