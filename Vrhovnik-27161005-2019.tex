...
\begin{dokaz}
$(\Rightarrow)$ Dokaz bo potekal z indukcijo na red Fareyevega zaporedja, na $n$.
...
Če je $b+d < n+1,$ je $\frac{a+c}{b+d}$ že nek člen v zaporedju $F_n$, zato uporabimo indukcijsko predpostavko. 
...
Zaradi simetrije je Fordove kroge dovolj obravnavati na intervalu $[0,1]$, obenem pa se zavedati, da jih lahko induktivno razširimo na celotno realno os. V nadaljevanju bomo s pojmom množica Fordovih krogov označevali množico Fordovih krogov z dotikališči na intervalu $[0,1]$.
...
\begin{figure}[h!]
\begin{center}
\includegraphics[scale=0.4]{pitagora1.png}
...
\begin{figure}[h!]
\begin{center}
\includegraphics[scale=0.5]{posploseni_fordovi_krogi.png}
\caption{Posplošeni Fordovi krogi, kjer je $\protect{h=\sqrt{2}/2}$.}
\label{Fig:PosplFord}
\end{center}
\end{figure}
...
\begin{figure}[h!]
\begin{center}
\includegraphics[scale=0.8]{fordove_krogle1.png}
...
\begin{opomba}
V izreku~\ref{izr:EulProdukt} nismo povedali, kateri množici pripada spremenljivka $s$. Euler je formulo namreč formuliral za celoštevilske $s$, Riemann pa je z razširitvijo funkcije zeta dokazal, da enakost velja za vse $s$, ki ustrezajo $Re(s)>1$.
\end{opomba}
...
Natančneje, $f(x) = o \left(g(x) \right)$, ko $x \to \infty$, če za vsak $\epsilon>0$ obstaja vrednost $x_{0}$, da za vsak $x \geq x_{0}$ velja $|f(x)| < \epsilon \cdot g(x)$.
\end{definicija}
