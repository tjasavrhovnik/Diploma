...
in preverimo, da je slika izbranega Fordovega kroga tudi Fordov krog. Res, središče slike je po enačbi~\ref{Eq:MobSlika} v točki
\begin{align}
...
Riemannova hipoteza, znana tudi kot 8.~Hilberov problem, je eno najbolj slavnih, še vedno nerešenih matematičnih vprašanj. Ime je dobila po nemškem matematiku Bernhardu Riemannu (17.\ 9.\ 1826 -- 20.\ 7.\ 1866), ki jo je formuliral med preučavenjem lastnosti velikih praštevil. Domnevo je leta 1859 zapisal v članku \emph{\"Uber die Anzahl der Primzahlen unter einer gegebenen Gr\"osse}. Na le osmih straneh je postavil temelje teorije, s katero se matematiki ukvarjajo vse od takrat in ima pomembne aplikacije na raznovrstnih področjih. 
...
\end{equation}
%
Relativno napako med $Li(x)=\int_{2}^x \frac{dt}{\log(t)}$ in $\pi(x)$ je v sredini 19.~stoletja izračunal Chebyshev, šele nekaj desetletij kasneje pa so Hadamard, von Mangoldt in de~la~Vall\'ee Poussin dokazali izrek o praštevilih in nekaj sorodnih trditev.
%
...
\begin{dokaz}
Spomnimo se funkcije $G$, podane z zvezo~\ref{Eq:Bernoulli}. Trdimo, da je soda. Oglejmo si novo funkcijo
\begin{align}
...
Trditev navajamo brez izpeljave -- ta se nahaja v \cite[poglavje 10.9]{zetafunction}. M\"obiusovo inverzijo bomo namreč potrebovali le v enem koraku dokaza napovedane leme, kjer bomo uporabili ekvivalenco~\ref{Eq:MobInv}.
...
\right.
\end{align}
Spomnimo se definicije Mertensove funkcije in upoštevajmo predpis~\ref{Eq:D(n)}. Dobimo
\begin{align*}
...
Primerjali bomo koeficiente pri $f(\frac{p}{q})$ na levi in desni strani enakosti~\ref{Eq:RiemFar1}.
...
%
Naj bo $\varepsilon > 0$. Naj bo $u \in [0,1]$ in definirajmo funkcijo $f(u) = e^{2\pi iu}$. Uporabimo lema~\ref{Lema:RiemFar} in funkcijo vstavimo v enakost~\ref{Eq:RiemFar1}; dobimo
\begin{equation}
\label{Eq:RiemFar2}
...
Enakost~\ref{Eq:RiemFar2} zato prepišemo v 
